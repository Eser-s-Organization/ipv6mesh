...
如果旧版 `.5`、`.6`、`.7`、`.8`、`.9`、`.10` 或 `.11` 弹出 `run Chinese IPv6Mesh UI: exit status 1`，请改用 `.12`。`.12` 同时包含 UTF-8 with BOM 修复、IPv6/端口自动生成 URL、Network ID/令牌随机生成与一键复制、可读的控制面健康错误、管理员令牌不匹配提示、控制面就绪等待、当前载荷优先、完整退出清理和 WireGuard 虚拟适配器删除。

如果创建网络或生成邀请时看到 `HTTP status 401 (unauthorized)` 或“管理员令牌无效或与控制面启动令牌不一致”，说明当前控制面可访问但令牌不匹配；`/healthz` 的 HTTP 200 只验证连接，不验证管理员身份。请填入启动该控制面时使用的同一个管理员令牌。如果是本机旧 `control-server.exe` 遗留进程，先停止旧进程，再在当前 UI 中用当前令牌点击“启动控制面”，看到健康检查通过后再创建网络。不要在控制面启动后重新生成令牌。

...
1. 在每台 Windows 电脑下载同一版本的 `ipv6mesh-installer-0.1.0-debug.10.exe`。
...
如果旧版 `.5`、`.6`、`.7`、`.8` 或 `.9` 弹出 `run Chinese IPv6Mesh UI: exit status 1`，请改用 `.10`。`.10` 同时包含 UTF-8 with BOM 修复、IPv6/端口自动生成 URL、Network ID/令牌随机生成与一键复制、可读的控制面健康错误、控制面就绪等待、完整退出清理和 WireGuard 虚拟适配器删除。

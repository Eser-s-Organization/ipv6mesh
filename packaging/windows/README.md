...
1. 在每台 Windows 电脑下载同一版本的 `ipv6mesh-installer-0.1.0-debug.7.exe`。
...
如果旧版 `.5` 或 `.6` 弹出 `run Chinese IPv6Mesh UI: exit status 1`，请改用 `.7`。`.7` 同时包含 UTF-8 with BOM 修复、IPv6/端口自动生成 URL 和完整退出清理。

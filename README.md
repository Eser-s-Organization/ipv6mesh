...
最简单的情况是：第一台电脑同时担任控制面管理员和游戏房主；第二台电脑是成员。普通成员只需要双击 `.exe`，不需要打开 PowerShell，也不需要使用 `vpnctl`。
...
1. 下载 [v0.1.0-debug.4 Windows 安装器](https://github.com/Eser-s-Organization/ipv6mesh/releases/download/v0.1.0-debug.4/ipv6mesh-installer-0.1.0-debug.4.exe)
2. 双击运行，Windows UAC 弹出后点击“是”
3. 按下表输入：

| 程序提示 | 房主输入 |
| --- | --- |
| `Control-plane URL` | `http://[2408:8256:1980:119b:f2c8:40bb:1e01:4182]:8080` |
| `One-time invite token` | `<host-one-time-token>` |
| `Device name` | `win11-host` |

4. 程序会自动停止旧服务、安装文件、启动服务、加入网络并连接虚拟网卡
5. 看到下面类似输出后，记下真实的 `Virtual IPv4`：
...
`10.42.0.2` 只是示例，必须使用程序实际显示的地址。如果房主电脑之前已经成功加入过网络，重新运行 `.4` 时程序会读取本机已有身份，通常不再要求新的邀请令牌。
...
| 程序提示 | 成员输入 |
| --- | --- |
| `Control-plane URL` | 与房主完全相同的控制面 URL |
| `One-time invite token` | `<member-one-time-token>` |
| `Device name` | `win11-member` |

4. 等待程序显示 `IPv6Mesh is connected`
5. 记下成员自己的 `Virtual IPv4`，例如 `10.42.0.3`
...
- **`open \\.\pipe\ipv6mesh`**：服务没有成功安装或没有运行，重新双击 `.4` 并确认 UAC 已授权
- **旧版本提示文件正在使用**：不要继续使用 `.2` 或 `.3`，使用 `.4`；`.4` 会先停止旧服务并等待文件解锁

...
最简单的情况是：第一台电脑同时担任控制面管理员和游戏房主；第二台电脑是成员。新版 `v0.1.0-debug.9` 将三种角色统一放在一个中文可视化界面中，普通成员只需要双击 `.exe`，不需要打开 PowerShell，也不需要使用 `vpnctl`。
...
下载 [v0.1.0-debug.9 Windows UI 安装器](https://github.com/Eser-s-Organization/ipv6mesh/releases/download/v0.1.0-debug.9/ipv6mesh-installer-0.1.0-debug.9.exe)。双击后在 UAC 对话框中点击“是”。程序会打开“IPv6Mesh 远程组网”窗口，顶部可以切换三种角色。SHA-256：`1E04EEBA65E8BE406DD10EEA7ED91FFFCA3CFC134695D302101E48D156420632`。

如果旧版 `.5`、`.6`、`.7` 或 `.8` 弹出 `run Chinese IPv6Mesh UI: exit status 1`，请改用 `.9`；`.9` 还会在控制面尚未启动或刚启动时给出明确提示，并提供令牌随机生成按钮。

| UI 角色 | 主要按钮和操作 |
| --- | --- |
| 控制面管理员 | 随机生成管理员令牌、启动控制面、检查健康、创建网络、随机生成房主/成员邀请、复制令牌 |
...
3. 在“管理员令牌”右侧点击“随机生成”，或粘贴一个由密码管理器生成的长随机令牌；令牌只保存在本次 UI 会话中，不写入日志。
...
7. 点击“随机生成房主邀请”和“随机生成成员邀请”，分别使用右侧按钮复制给房主和成员。令牌由控制面使用密码学安全随机数生成，并同时保存令牌哈希；UI 只在专用框中显示一次性令牌，不写入日志。
...
1. 下载 [v0.1.0-debug.9 Windows UI 安装器](https://github.com/Eser-s-Organization/ipv6mesh/releases/download/v0.1.0-debug.9/ipv6mesh-installer-0.1.0-debug.9.exe)
...
- **`open \\.\pipe\ipv6mesh`**：服务没有成功安装或没有运行，重新双击 `.9` 并确认 UAC 已授权；可在 UI 中点击“刷新状态”查看日志
- **旧版本提示文件正在使用**：使用 `.9`；安装脚本会先停止旧服务并等待文件解锁，UI 关闭时也会停止服务并清理虚拟网络资源
